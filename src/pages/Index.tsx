...
import { useMintDrop, usePlaceBid } from "@/hooks/useContracts";
...
const Index = () => {
  ...
  const { mint, isPending: isMinting, error: mintError, isSuccess: isMintSuccess } = useMintDrop();
  ...
  useEffect(() => {
    if (supabaseLiveDrops && supabaseLiveDrops.length > 0) {
      setLiveDrops(supabaseLiveDrops.map((drop) => {
        ...
        // Only include drops that have been created on-chain with a contract_drop_id
        if (!drop.contract_drop_id) {
          console.warn(`⚠️ Drop "${drop.title}" missing contract_drop_id - skipping mint`);
          return null;
        }
        return {
          id: drop.id,
  ...
  const handleBuyDrop = async (drop: any) => {
    ...
    try {
      // Must use contractDropId (the on-chain drop ID), not the database UUID
      if (!drop.contractDropId) {
        throw new Error("This drop is not linked to a contract yet");
      }
      ...
      console.log(`🛒 Minting contract drop #${drop.contractDropId} for ${drop.priceEth} ETH (${priceWei} wei)...`, {
  ...
  useEffect(() => {
    if (mintError && mintingDropId) {
      ...
      const errMsg = mintError.message || "Transaction was rejected";
  ...
  useEffect(() => {
    if (isMintSuccess && mintingDropId) {
  ...
  }, [isMintSuccess, mintingDropId, toast, refetchDrops]);
                        ...
                        <div className="flex gap-2 mt-3">
                          {drop.type === "auction" ? (
                            ...
                            <Button
                              ...
                              disabled={isMinting || mintingDropId === drop.id}
                              className="flex-1 h-8 rounded-full gradient-primary text-primary-foreground font-semibold text-xs"
                            >
