import { getStoredArtistWhitelist } from "@/lib/whitelist";
import {
...
// Hybrid approach: try Supabase, fallback to localStorage
...
export function getAllArtists() {
  const artists = _artistsCache || loadState(STORAGE_KEYS.artists, EMPTY_ARTISTS);
...
export function getAllDrops() {
  const drops = _dropsCache || loadState(STORAGE_KEYS.drops, EMPTY_DROPS);
...
export async function updateArtistDropContractId(dropId: string, contractDropId: number) {
  try {
    ...
    if (updated) {
      ...
      _dropsCache = (_dropsCache || loadState(STORAGE_KEYS.drops, EMPTY_DROPS)).map((drop) =>
        drop.id === dropId ? { ...drop, contractDropId, contractKind: "artDrop" } : drop
      );
      saveState(STORAGE_KEYS.drops, _dropsCache);
      return updated;
    }
...
export function getAllCampaigns() {
  return _campaignsCache || loadState(STORAGE_KEYS.campaigns, EMPTY_CAMPAIGNS);
}
...
export async function saveArtistRecord(record: ArtistPublicProfile) {
  ...
  _artistsCache = next;
  saveState(STORAGE_KEYS.artists, next);
...
export function createArtistDrop(drop: ArtistDropRecord) {
  ...
  _dropsCache = next;
  saveState(STORAGE_KEYS.drops, next);
...
export function createArtistCampaign(campaign: ArtistCampaignRecord) {
  const campaigns = _campaignsCache || loadState(STORAGE_KEYS.campaigns, EMPTY_CAMPAIGNS);
  const next = [campaign, ...campaigns.filter((existing) => existing.id !== campaign.id)];

  _campaignsCache = next;
  saveState(STORAGE_KEYS.campaigns, next);
...
export async function initializeFromSupabase() {
  ...
  try {
    ...
    // Load artists from Supabase and merge with localStorage
    const dbArtists = await Promise.all(
      getApprovedWallets().values().map(async (wallet) => {
        try {
    ...
    const localArtists = loadState(STORAGE_KEYS.artists, EMPTY_ARTISTS);

    // Merge: DB takes precedence, but keep local-only artists
    const merged = [
      ...validDbArtists,
      ...localArtists.filter(
        (local) =>
          !validDbArtists.find((db) => normalizeWallet(db.wallet) === normalizeWallet(local.wallet))
      ),
    ];

    _artistsCache = merged;
    saveState(STORAGE_KEYS.artists, merged);

    console.log(`✅ Loaded ${merged.length} artists from mixed sources`);
  } catch (error) {
