...
function cacheArtistWhitelist(entries: ArtistWhitelistEntry[]) {
  if (typeof window === "undefined") return;

  try {
    window.localStorage.setItem(ARTIST_WHITELIST_STORAGE_KEY, JSON.stringify(entries));
  } catch {
    // Cache failure should not change authorization behavior.
  }
}
...
export function getStoredArtistWhitelist(): ArtistWhitelistEntry[] {
  ...
  try {
    ...
    return dedupeEntries(
      list
        .map((entry) => normalizeEntry(entry as Partial<ArtistWhitelistEntry>))
...
export function syncArtistWhitelist(entries: ArtistWhitelistEntry[]) {
  ...
  cacheArtistWhitelist(normalized);
  return normalized;
}
